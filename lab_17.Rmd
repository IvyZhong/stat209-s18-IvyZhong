...
**Answer**:
30% percent of the cereal has the health score below 34.5
...
**Answer**:
Nabisco has the healthiest cereals.
...
**Answer**:
You should look at the top shelf. According to the graph, the top shelf has a very high health score and the second high sugar amount in them.
...
**Answer**:
80% of tea has price below 30 dollars. 
...
**Answer**:
They usually has less reviews since most of the purple dots (represented by the above 30 dollars expensive teas.) The score also varies from low to high.
...
**Answer**:
The variation of the white tea prices smaller than the price of all teas.
...
ggplot(tea_type, aes(price_mean, score_mean)) +
  ...
  geom_text_repel(aes(label = type), size = 5)
```
...
**Answer**:
The matcha has a high price and a low score while others have a low price and a relatively higher score.
