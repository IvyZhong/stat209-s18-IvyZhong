...
```{r}
tweets_user1 <- filter(tweets, user == "amyklobuchar")
tweets_user1in2017 <- filter(tweets, between(date, as.POSIXct("2016-12-31"), as.POSIXct("2017-12-31")))
```

This is the number of tweets for user "amyklobuchar" has in 2017.

```{r}
ggplot(tweets_user1, aes(date, retweets)) +
  geom_point() +
  geom_point(color = "red", data = tweets_user1in2017)
```

This is the number of tweets for user "amyklobuchar" has in 2016.
```{r}
tweets_user1 <- filter(tweets, user == "amyklobuchar")
tweets_user1in2016 <- filter(tweets, between(date, as.POSIXct("2015-12-31"), as.POSIXct("2016-12-31")))
```

```{r}
ggplot(tweets_user1, aes(date, retweets)) +
  geom_point() +
  geom_point(color = "purple", data = tweets_user1in2016)
```

This is the number of tweets for user "amyklobuchar" has in 2015.

```{r}
tweets_user1 <- filter(tweets, user == "amyklobuchar")
tweets_user1in2015 <- filter(tweets, between(date, as.POSIXct("2014-12-31"), as.POSIXct("2015-12-31")))
```

```{r}
ggplot(tweets_user1, aes(date, retweets)) +
  geom_point() +
  geom_point(color = "pink", data = tweets_user1in2015)
```
