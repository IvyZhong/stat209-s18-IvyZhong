...
It can be seen that amyklobuchar retweets more than anytime before 2017. 
...
```{r}
ggplot(tweets, aes(party, date)) +
  geom_point()
```

```{r}
ggplot(tweets, aes(state, retweets)) +
  geom_point()
```

```{r}
tweets_retweets <- filter(tweets, retweets >= 100, favorites >= 100)
```
