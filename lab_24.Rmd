---
title: "Lab 24: Chicago Crime Data II"
...
There are 141064 obversations in the dataset crimes. One row represents a crime.
The plot shows, whether the crime happens during night or day, the higher the income, the lower the number of burglary. Since the line for day time is always lower than the line for night time, the plot shows that there are more burglary happened during night time. And the confidence level for night time data does not overlap the day time confidence level for most of the income, there is a statistically significant difference present.
The slope difference between the two categorical variables means the higher the income, the faster the number of burglary decreased.  
...
```{r}
crimes_nar <- filter(crimes, narcotics == 1)
crimes_narsum <- group_summarize(crimes_nar, area_number)
crimes_narsumleft <- left_join(crimes_narsum, ca, by = "area_number")

ggplot(crimes_narsumleft, aes(median_income, arrest_flag_mean)) +
  geom_point() +
  geom_smooth(method = "lm")
```

Almost all of the narcotics are arrested in all neighborhoods.
