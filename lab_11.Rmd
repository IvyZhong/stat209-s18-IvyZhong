...
Answer: The relationship between the mass of male and the mass of egg has a positive correlation.
...
Answer:Yes, the line of best fit did show a positive correlation between the two variables.
...
ggplot(birds, aes(male_mass, egg_mass)) +
  ...
  scale_y_log10() +
  geom_smooth(method = "lm", color = "black", se = FALSE, linetype = "dashed", size = 0.5) +
  annotate("point", x = 87.00, y = 218.7, color = "red") +
  annotate("text", x = 87.00, y = 165, label = "Tropicbird", size = 3)
```
...
```{r}
owl <- filter(birds, type == "Owl")
```

```{r}
honeyeater <- filter(birds, type == "Honeyeater")
```
...
ggplot(birds, aes(male_mass, egg_mass)) +
  ...
  scale_x_log10() +
  scale_y_log10() +
  geom_point(color = "purple", data = owl) +
  geom_point(color = "yellow", data = honeyeater) +
  geom_point(color = "red", data = parrots) +
  geom_smooth(method = "lm", color = "black", se = FALSE, linetype = "dashed", size = 0.5) +
  ggtitle("Relationship between male mass and egg size", "purple = owl, yellow = honeyeater, red = parrots") +
  annotate("point", x = 3.4, y = 0.52, color = "blue") +
  annotate("text", x = 3.9, y = 0.4, label = "Ruby-throated", size = 3)
```
