...
```{r}
model1 <- lm_basic(base~1, data = teachers)
```
...
**Answer**:
Yes if the estimated mean value is rounded to the nearest whole number. 
Mean is 56937.61 which is about 59638.
...
**Answer**:
No, since there are larger counts above the 64000 and some other data below 54000.
...
reg_table(model3, level = 0.8)
```

Make sure you actually extract the answer here:

**Answer**:
33.86 average, within the range of 33.07 and 34.65

Describe in words how the proceeding confidence interval should be
interpreted.

**Answer**:
The data has an 80% of confidence of correctness. 
