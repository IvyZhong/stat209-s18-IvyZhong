...
```{r}
model_per <- lm_basic(perfectionism ~ 1 + speed, data = pf)
reg_table(model_per, level = 0.95)
```
...
**Answer**: Normal and slow are all bigger than the faster. They are both significant.
...
model_per <- lm_basic(perfectionism ~ 1 + speed_normal, data = pf)
reg_table(model_per, level = 0.95)
```
...
**Answer**: People have higher perfectionism usually use higher amount of time.
...
```{r}
model_warm <- lm_basic(warmth ~ 1 + dominance + sensitivity, data = pf)
reg_table(model_warm, level = 0.95)
```
...
**Answer**: The dominance is higher which is not what expected. The sensitivity is higher which matches my expectation.

Take the previous model and add the variables `speed`, `gender`, and
`country`, the latter lumped into 5 categories.

```{r}
model_multi <- lm_basic(warmth ~ 1 + dominance + sensitivity + speed + gender + fct_lump(country, 5), data = pf)
reg_table(model_multi, level = 0.95)
```
...
**Answer**:  The diminance and sensitivity is statistically higher than the warmth.
