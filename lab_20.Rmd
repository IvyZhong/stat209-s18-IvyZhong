...
```{r}
model_weight <- lm_basic(price ~ 1 + carat, data = diamonds)
reg_table(model_weight)
```

How would you interpret the slope coefficient in this model?

**Answer**: 
One additional carat of diamond would add 7756 price on it.
...
```{r}
model_weight <- lm_basic(price ~ 1 + carat, data = diamonds)
reg_table(model_weight, level = 0.95)
```
...
**Answer**:
The interval of slope are both positive, so the interval is in a small range and in consistence with the base value (a positive relationship).
...
**Answer**:
The greater the depth of one unit, the price decreases by 29.65
...
**Answer**:
We are confident that the relationship is negative.
...
**Answer**:
Yes. Depth has the intervals both in negative signs indicates the negative relationship. Carat has both intercals in the positive signs indicates the positive relationship.
...
**Answer**:
The slope of the depth: the price change when each unit of depth increases with a fixed amount of weight.
The slope of the weight: the price change when each unit of weight increases with a fixed amount of depth
