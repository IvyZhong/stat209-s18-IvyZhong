...
**Answer**: The bar plot indicates most people get score of 15, and the number of people getting each score gets larger at first, and gets smaller as the score gets above 15.
Since the score is a catagorical variable, it has 20 catagories.
...
model_HU <- lm_basic(warmth ~ 1, data = HU)
...
**Answer**: The confidence intervals indicate the significant difference between the two data set. The data for Warmth in Hungary is lower than the average level of the whole data set.
...
```{r}
model_HU <- lm_basic(warmth ~ 1 + gender, data = HU)
reg_table(model_HU, level = 0.95)
```
...
**Answer**: No. The confidence level does not include 0. Therefore, no statistically signicant difference.
...
```{r}
age <- filter(pf, between(age, 30, 49))
```
...
model_age <- lm_basic(warmth ~ 1 + fourties, data = age)
reg_table(model_age, level = 0.95)
```
...
**Answer**: Yes. Fourties tends to have a higher score of Warmth.This confirm traditional age stereotypes.
...
```{r}
model_country <- lm_basic(warmth ~ 1 + fct_lump(country, 5), data = pf)
...
**Answer**: Australia. Other (combined) has the lowest.
...
```{r}
model_sens <- lm_basic(baseline ~ 1 + sensitivity, data = pf)
reg_table(model_sens, level = 0.95)
```

Is the slope statistically significantly different from 1? Why is this an
interesting question in the context of the data?

**Answer**: Yes. This means that the data for sensitivity does not have the same trend as the baseline changes.

Fit a regression model that uses both gender and the baseline score to
predict your trait.

```{r}
model_sensgen <- lm_basic(baseline ~ 1 + sensitivity + gender, data = pf)
reg_table(model_sensgen, level = 0.95)
```

Describe the slope for the gender term in words:

**Answer**: For one more increase in the baseline variable, the sensitivity score increase in male by 0.1508

Add predictions from the previous model to `pf`.

```{r}
pf <- add_prediction(pf, model_sensgen)
model_sensgen <- lm_basic(baseline ~ 1 + sensitivity + gender, data = pf)
model_sensgen
```
...
ggplot(pf, aes(baseline, sensitivity)) +
  geom_point() +
  geom_point(aes(y = model_sensgen_pred, color = gender)) +
  geom_line(aes(y = model_sensgen_pred, color = gender))
```
