...
```{r}
birdtype <- group_summarize(birds, type)
```
...
```{r}
ggplot(birdtype, aes(egg_mass_median, male_mass_median)) +
  geom_point() +
  scale_x_log10() +
  scale_y_log10()
```
...
```{r, echo = TRUE}
birdmorethan5 <- filter(birdtype, n >=5)
```
...
ggplot(birdtype, aes(egg_mass_median, male_mass_median)) +
  geom_point()+
  geom_point(color = "red", data = birdmorethan5) +
  geom_text_repel(data = birdmorethan5, aes(label = type), alpha = 0.8) +
  scale_x_log10() +
  scale_y_log10() +
  ylab("Median of Male Mass") +
  xlab("Median of Egg Mass")
```
