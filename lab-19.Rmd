...
**Answer**:
It is higher for burrito restaurants. The average is 3.9 and the lower range is higher than 3.7
...
**Answer**:
The data is biased and only picks the higher score data.

Now fit a linear regression that predicts the yelp score as a function of
whether the burrito includes chips.

```{r}
model_chips <- lm_basic(yelp ~ 1 + chips_included, data = burrito)
reg_table(model_chips, level = 0.95)
```
...
**Answer**:
If chips are included, the average yelp score increases by 0.6.
...
**Answer**:
The slope coefficient in the previous model lies in between the chips included.
...
```{r}
model_synergy <- lm_basic(yelp ~ 1 + high_synergy, data = burrito)
reg_table(model_synergy, level = 0.95)
```
...
**Answer**:
No, because the range for the higher synergy includes the value of the low synergy.
...
**Answer**:
Yes, the upper range and the lower range are both higher. 
...
```{r}
burrito$synergy_true <- fct_relevel(burrito$high_synergy, "TRUE")
model_so <- lm_basic(overall ~ 1 + synergy_true, data = burrito)
reg_table(model_so, level = 0.95)
```
